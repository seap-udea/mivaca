...
interface PayPalDonateProps {
  email?: string;
  hostedButtonId?: string; // ID del botón de PayPal si ya tienes uno creado
  className?: string;
}
...
export default function PayPalDonate({ 
  ...
  // Si tienes un hosted_button_id de PayPal, úsalo (más profesional)
  // Si no, usa el email para crear un enlace de donación
  const paypalUrl = hostedButtonId 
    ? `https://www.paypal.com/donate/?hosted_button_id=${hostedButtonId}`
    : `https://www.paypal.com/donate?business=${encodeURIComponent(email)}&currency_code=USD&item_name=Donación+para+Mi+Vaca`;
    ...
    <div className={`${className}`}>
      {!showDonate ? (
        <button
          ...
          aria-label="Mostrar opciones de donación"
        >
